...
class AutoregTreeGenMB(pl.LightningModule):
    def __init__(
        ...
        training_args: Optional[Union[ConfigDict, Dict]] = None,
        ...
        if self.training_args is None:
            self.training_args = ConfigDict()
            self.training_args.use_desc_mass_ratio = False
    ...
    def _prepare_batch(self, batch):
        ...
        x = feat[:, :max_len, :self.d_in].clone()
        t = feat[:, :max_len, self.d_in:].clone()
        ...
        if self.training_args.use_desc_mass_ratio:
            tgt = tgt.clone()
            tgt[:, :, 0] = tgt[:, :, 0] - src[:, :, 0]
